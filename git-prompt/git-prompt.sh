...
#################################################################### WHO_WHERE
        ...
        if [[ -n $id  || -n $host ]] ;   then
                [[ -n $id  &&  -n $host ]]  &&  at='@'  || at=''
                color_who_where="${id}${host:+$host_color$at$host}"
...
prompt_command_function() {
	...
	parse_virtualenv_status
        ...
        PS1="$colors_reset$rc$head_local$color_who_where$dir_color$cwd$tail_local$dir_color$colors_reset $prompt_char "

        unset head_local tail_local pwd
 }

        PROMPT_COMMAND="prompt_command_function; history -a; history -c; history -r;"
