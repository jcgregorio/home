...
function! PyGetDocstringFromArgs(text)
    ...
    let docvars = map(split(text), 'v:val.": ".st.et.", ".st.et')
    return '\n\n'.indent.join(docvars, '\n'.indent).'\n'.indent
endfunction
...
\<CR>\"\"\"".st.et.st."args:PyGetDocstringFromArgs(@z)".et."\"\"\"
\<CR>".st.et
...
\<CR>\"\"\"Constructor.".st."args:PyGetDocstringFromArgs(@z)".et."\"\"\"
...
exec "Snippet ifm if __name__ == '__main__':<CR>".st.et
